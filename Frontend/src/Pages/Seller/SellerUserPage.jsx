...
const SellerProfile = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      // Assuming the response structure is { data: { ...userDetails } }
      setUser(response.data); // Set the entire user object
      form.setFieldsValue({
  ...
  const onFinish = async (values) => {
    try {
      ...
      setUser(response.data); // Update with new user data
  ...
  return (
    <div className="max-w-lg p-8 mx-auto bg-white rounded-lg shadow-xl">
      ...
      <div className="mb-6 space-y-2">
        ...
        <p className="text-gray-600">
          <strong className="font-medium text-gray-800">Farm Name:</strong>{" "}
          {user.farm_name}
        </p>
        <p className="text-gray-600">
          <strong className="font-medium text-gray-800">Farm Address:</strong>{" "}
          {user.farm_address}
        </p>
      </div>
